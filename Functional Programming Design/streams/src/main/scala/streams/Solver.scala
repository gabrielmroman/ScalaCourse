...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    ...
    def nWH(n : List[(Block, Move)]):Stream[(Block, List[Move])] = n match {
      case Nil => Stream.empty
      case x::xs => (x._1, x._2 :: history) #:: nWH(xs)
    }
    nWH(n)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial isEmpty) Stream.empty
      ...
      initial #::: from(more.toStream, explored union more.map(x => x._1).toSet)
    }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    from(Pair(startBlock, List[Move]()) #:: Stream.empty, Set.empty)
  }
